...
function loadBrowsingProfiles(){
	...
	for(var u = 0; u < size; u++){
		if(userDatabase[u].uid != userID){
			console.log(userDatabase[u].uid)
			if(getProfileById(userDatabase[u].uid, alreadyLiked) === 'not found'){
...
function getProfileById(targetID, customUserList){
	var response = 'not found';
	var userList = customUserList || userDatabase;
	var size = userList.length;
	for(var u = 0; u < size; u++){
		if(userList[u].uid === targetID){
			response = userList[u];
			break;
		}
	}
