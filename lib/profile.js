...
function userFactory(data){
	var user = {
		...
		getProfileDiv: function(){
			var size;
			var html = '<div id="profile-' + this.uid + '" class="people">';
...
/*---> BROWSING <-----------------------------*/
/*--------------------------------------------*/
//https://tinderpreneurship.firebaseio.com/users/103105083532765828171/profile
function likeProfile(likeUID){
	var path = "https://tinderpreneurship.firebaseio.com/users/" + likeUID + "/profile/";
	var likesRef = new Firebase(path + "likedyou");
	likesRef.once("value", function(snapshot){
		var data = JSON.parse(snapshot.val());
		data.push(userID);
		likesRef.set(JSON.stringify(data));
		document.getElementById('profile-' + likeUID).style.display = 'none';
	});
}
